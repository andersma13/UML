...
public class Main extends Application
{
	...
	/**
	 * Model: this class will handle all objects in the program.  Data can be
	 * pulled from it at will and pushed to the view.
	 */
	static Model data = new Model();
	
	/**
	 * View: This class displays the data for user interaction.  It holds a
	 * reference to the model to make passing information back into the model
	 * cleaner and easier.
	 */
	...
	public void start(Stage primaryStage)
	{
		 ...
		 * Whenever the list of Classes is updated, that class block can be 
		 * redrawn while leaving the others untouched.  New class blocks can be
		 * added and removed at will.
		 */
		data.getClassProperty().addListener(classListener());
		
		/**
		 * Listen for changes to the Link model and update the view accordingly.
		 * At the moment, only listens for new links.  Removing links is done by
		 * the classListener, because currently links cannot be removed on their
		 * own.  When we implement a method to remove them on their own, this
		 * listener can be modified to perform that action.
		 */
							...
							for(ClassModel added : c.getAddedSubList())
							{
								...
								newClass.setOnMouseDragged(new EventHandler<MouseEvent> ()
								{
									...
									public void handle(MouseEvent e)
									{
										...
										newClass.getNode().setX((int) (added.getXPos() +
												(newClass.getWidth() / 2) + e.getX() - delta.x));
										newClass.getNode().setY((int) (added.getYPos() +
												(newClass.getHeight() / 2) + e.getY() - delta.y));
					...
					else
					{
						if(c.wasAdded())
						{
							for(LinkModel added : c.getAddedSubList())
							{
								...
								data.getClass(srcIndex).getNode().getXProperty().addListener(
								...
								data.getClass(srcIndex).getNode().getYProperty().addListener(
										new ChangeListener<Number>()
								{
									@Override
									public void changed(ObservableValue<? extends Number> observable, 
								...
								data.getClass(destIndex).getNode().getXProperty().addListener(
										new ChangeListener<Number>()
								{
									@Override
									public void changed(ObservableValue<? extends Number> observable, 
								...
								data.getClass(destIndex).getNode().getYProperty().addListener(
										new ChangeListener<Number>()
								{
									@Override
									public void changed(ObservableValue<? extends Number> observable, 
	...
	*/
	 ...
	 * 
	 * @param in	The Class Block to be added
	 */
